...
export const downloadSong = (hash) => (dispatch, getState) => {
  let state = { ...getState() }
  if(state.songs.downloadingCount >= 3) {
    dispatch({
      type: SET_WAIT_LIST,
      payload: [...state.songs.waitingToDownload, hash]
    })
    return
  }
  dispatch({
      ...
      if(results.songs.length === 1) {
        ...
        let req = request.get({
          ...
          dispatch({
            ...
            payload: [...getState().songs.downloadedSongs, {hash: hash, file: path.join(getState().settings.installationDirectory, 'CustomSongs', extractTo, infoEntry.entryName)}]
          })
          state = { ...getState() }
          dispatch({
            type: SET_DOWNLOADING_COUNT,
...
export const deleteSong = (file) => (dispatch, getState) => {
  dispatch({
    type: SET_VIEW,
